...
export default class TaskController implements ApplicationController {
  ...
  private async get(req: Request, res: Response): Promise<Response> {
    try {
      ...
      console.log(
        `Ends retriving task ${req.params.id} with response: ${task.toJSON()}`
      );
      return res.send(task);
    } catch (e: ControlledException | any) {
      console.warn(
        `The following error ${e} occurred while getting task ${req.params.id}`
      );
      res.status(e.status | 404);
      return res.json({ message: e.message });
    }
  ...
  private async create(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const task = await new TaskCreator().create({
        ...req.body,
        ...req.files,
      });
      ...
      console.warn(
        `The following error ${e} occurred while trying to create task ${req.body}`
      );
      res.status(e.status | 404);
