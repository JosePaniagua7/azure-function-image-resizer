...
export default class SubscribersNotifier {
    ...
    async notify(imageId: string, source: Buffer) {
        ...
        const subscribersEndpoints = this.subscribers.map((endpoint: string) => `${endpoint}/${imageId}`);
        const notificationPromises = subscribersEndpoints.map((endpoint) => axios.post(endpoint, form));
